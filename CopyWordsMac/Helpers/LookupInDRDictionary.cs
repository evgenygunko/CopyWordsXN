...
{
    public class LookupInDRDictionary
    {
        ...
        public ScannedImageViewModel CreateViewModel(string word)
        {
            ...
            {
                AlertManager.ShowWarningAlert("Cannot find scanned file", $"Cannot find a scanned page for the word '{word}'.");
                return null;
            }

            List<string> allPages = new List<string>();

            // first file name is: 0013.jpg
            for (int i = 13, j = 0; j < _wordsMapApproxDict.Count; i++, j++)
            {
                allPages.Add(string.Format("{0}.jpg", i.ToString().PadLeft(4, '0')));
            }

            ScannedImageViewModel vm = new ScannedImageViewModel(word, allPages, fileName);
        ...
        internal string GetFileName(string word)
        {
            ...
            {
                return _wordsMapAccurateList[0].FileName;
            }
            ...
            {
                if (word.StartsWith(item.Value, StringComparison.InvariantCulture))
                {
